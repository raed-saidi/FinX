...
def _parse_allowed_origins() -> List[str]:
    """Parse CORS origins from env; defaults to localhost-only for safety."""
...
print("🔍 GROQ_API_KEY: configured" if GROQ_API_KEY else "🔍 GROQ_API_KEY: NOT SET")
groq_client = None
...
LEGACY_SALT = "finx_secure_salt_2024"

def hash_password(password: str) -> str:
    """Hash password with PBKDF2-HMAC-SHA256 using unique per-user salt."""
...
def verify_password(password: str, stored_hash: str) -> bool:
    """Verify modern PBKDF2 hashes and legacy SHA256 hashes."""
    ...
    # Backward compatibility for existing legacy users.
    legacy = hashlib.sha256(f"{LEGACY_SALT}{password}".encode()).hexdigest()
    return hmac.compare_digest(legacy, stored_hash)


def _is_session_expired(token: str) -> bool:
    """Check whether a token session is expired or invalid."""
    session = active_sessions.get(token)
    if not session:
        return True

    timestamp = session.get("last_active") or session.get("created_at")
    if not timestamp:
        return True
    ...
    return datetime.now() - last_active > SESSION_TIMEOUT
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if token in tokens_db and not _is_session_expired(token):
        # Update last active time
        if token in active_sessions:
            active_sessions[token]['last_active'] = datetime.now().isoformat()
        return tokens_db[token]

    # Cleanup stale or invalid sessions/tokens.
...
@app.post("/predict")
def predict_signal(payload: PredictRequest):
    """Return the latest prediction payload for a requested symbol."""
    symbol = payload.symbol.upper().strip()
    if not symbol:
        raise HTTPException(status_code=400, detail="Symbol is required")

    recommendations = load_recommendations()
    match = next((item for item in recommendations if item.get("asset") == symbol), None)
    if not match:
    ...
    return {
        ...
        "confidence": abs(float(match.get("signal", 0.0))),
...
def login(credentials: UserLogin, request: Request):
    ...
    # Upgrade legacy password hashes after successful login.
...
def execute_trade(trade: TradeRequest, user: dict = Depends(get_current_user)):
    ...
    # If broker is configured, submit first to avoid local state drift on failures.
    alpaca_order = None
    if alpaca_trading_available and alpaca_client:
        try:
            from alpaca.trading.requests import MarketOrderRequest
...
async def start_bot(config: Optional[BotConfig] = None, user: dict = Depends(get_current_user)):
...
def update_bot_config(config: BotConfig, user: dict = Depends(get_current_user)):
...
def get_bot_status(user: dict = Depends(get_current_user)):
    """Get bot status and Alpaca account info for authenticated users."""
    if user is None:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
# ============== Direct Alpaca Trading (Authenticated) ==============
...
@app.post("/api/trade")
def execute_alpaca_trade(trade: AlpacaTradeRequest, user: dict = Depends(get_current_user)):
    """Execute a trade directly through Alpaca for authenticated users."""
    if user is None:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
def execute_batch_trade(
    ...
    """Execute batch trades using AI signals or custom symbols for authenticated users."""
    if user is None:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
def get_alpaca_positions(user: dict = Depends(get_current_user)):
    """Get real positions from Alpaca for authenticated users."""
    if user is None:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
def get_alpaca_orders(user: dict = Depends(get_current_user)):
    """Get recent orders from Alpaca for authenticated users."""
    if user is None:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if not token or token not in tokens_db or _is_session_expired(token):
